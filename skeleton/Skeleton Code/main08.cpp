...
int main() {
    ...
    if (testCase == 8) //test case derived from the provided skeleton main function
    {
        ...
        Mode mode = modes[0]; //different mode for different test cases
        HashTable<int, int> ht(7, fun, fun2, mode, 0.5);

        for (int i = 0; i < sizeof(values) / sizeof(int); i++) {
            ht.add(keys[i], new int(values[i]));
            cout << "insert key: " << keys[i] << endl;
            cout << ht << endl;
        }
